...
$sql = "insert into case_order values(null,'$userid','$phone','$user_name','$order_time','$addr','$totalprice')";
...
if($result){    //INSERT语句执行成功，需要获取新产生的订单id
    ...
    foreach ($cart as &$one ) {
        //将数据插入到购物车详情
        $sql = "insert into case_orderdetails values('$oid','$one->did','$one->dishCount','$one->price')";
        $result = mysqli_query($conn, $sql);
        //从购物车中删除
        $sql = "DELETE FROM case_cart WHERE ctid=$one->ctid";
        $result = mysqli_query($conn,$sql);
    }
